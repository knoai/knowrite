...
function encryptKey(key) {
  ...
  if (!getEncryptionKey()) {
    console.warn('[settings-store] ⚠️ ENCRYPTION_KEY 未设置，API Key 将以明文形式存储。生产环境强烈建议设置 ENCRYPTION_KEY');
    return key;
  }
  ...
  try {
    ...
    throw new Error('AES 加密失败: ' + err.message);
  }
}
...
function decryptKey(encKey) {
  ...
  try {
    ...
    throw new Error('AES 解密失败: ' + err.message);
  }
}
...
/** 返回给客户端的安全版本，屏蔽 apiKey */
async function getSafeModelConfig() {
  const cfg = await getModelConfig();
  const safe = JSON.parse(JSON.stringify(cfg));
  if (safe.providers) {
    for (const pk of Object.keys(safe.providers)) {
      const p = safe.providers[pk];
      if (p && p.apiKey) {
        p.apiKey = p.apiKey ? '******' : '';
      }
    }
