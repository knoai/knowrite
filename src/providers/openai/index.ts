...
class OpenAIProvider extends BaseProvider {
  ...
  constructor(options: Record<string, unknown> = {}, name: string = 'openai') {
    super(name, options);
    this.apiKey = (options.apiKey as string) || '';
    this.baseURL = ((options.baseURL as string) || '').replace(/\/$/, '');
  }
  ...
  async chat(messages: ChatMessage[], options: ChatOptions = {}): Promise<string | AsyncGenerator<string, void, unknown>> {
  ...
  async _request(url: string, body: any, stream: boolean = false): Promise<any> {
    const netCfg: any = await getConfig('network');
    const config = {
      headers: {
  ...
  async _chatSync(url: string, body: any): Promise<string> {
    const resp = await this._request(url, body, false);
    return resp.data?.choices?.[0]?.message?.content || '';
  }

  async *_chatStream(url: string, body: any): AsyncGenerator<string, void, unknown> {
  ...
  async embed(texts: string | string[], options: Record<string, unknown> = {}): Promise<number[][]> {
    const model = (options.model as string) || 'text-embedding-3-small';
